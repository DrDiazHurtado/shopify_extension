let isPro = false;
let allProducts = [];
const API_URL = "https://tu-radar-api.vercel.app/api/sync"; // Tu API en Vercel
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const jsonUrl = `${url.origin}/products.json?limit=250`;
  ...
  try {
    ...
    // Calculate basic metrics (Simplified from your Python logic)
    ...
    // 3. Sync with Central Database (Vercel + Supabase)
    const syncResponse = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        domain: url.hostname,
        data: allProducts.slice(0, 10), // Limit payload for speed
        metrics: metrics
      })
    });
    ...
    // Update Ranking UI (The "Free" hook)
    ...
    upgradeBtn.onclick = () => {
        // Enlazar a Stripe Checkout
        alert("Redirecting to Stripe: €3 Unlock Competitor List");
    };
